/**
 * EvolutionStats — Analytics dashboard for the Evolution Practicum.
...
import type { EvolCourseData } from './types'
import { TOPIC_LABELS, TOPIC_COLORS } from './types'

interface Props {
  courseData: EvolCourseData
  onBack: () => void
}
...
function buildHeatmap(history: EvolCourseData['sessionHistory']): HeatCell[] {
...
function cellColor(count: number): string {
  ...
  if (count === 1) return 'rgba(167,139,250,0.20)'
  if (count <= 3) return 'rgba(167,139,250,0.50)'
  return 'rgba(167,139,250,1.00)'
}

function ActivityHeatmap({ history }: { history: EvolCourseData['sessionHistory'] }) {
...
function TopicHeatmap({ history }: { history: EvolCourseData['sessionHistory'] }) {
...
export default function EvolutionStats({ courseData, onBack }: Props) {
  ...
  return (
    <div style={{ maxWidth: 720, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 16 }}>
        <button onClick={onBack} style={{ background: 'none', border: 'none', color: '#a78bfa', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', padding: 0 }}>
          ← Back
        </button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 20 }}>
        {[
          { label: 'Total Sessions', value: overview.totalSessions, color: '#a78bfa' },
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {mostMissed.map(q => (
              <div key={q.id} style={{
                ...
                <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 6, background: '#a78bfa22', color: '#a78bfa', border: '1px solid #a78bfa', flexShrink: 0 }}>
      ...
      <div style={{ marginTop: 8, marginBottom: 24 }}>
        {storageMB > 4 && (
          <div style={{ padding: '8px 12px', borderRadius: 8, marginBottom: 8, background: '#a78bfa22', border: '1px solid #a78bfa', fontSize: 12, color: '#a78bfa', fontWeight: 600 }}>
