/**
 * EvolutionResults — Session summary with score breakdown, topic chart, per-answer accordion.
 */
...
function AnswerRow({ answer, question, index }: { answer: EvolAnswer; question: EvolQuestion | undefined; index: number }) {
    ...
    <div style={{
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2, display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            <span>{question ? TOPIC_LABELS[question.topic] : '—'}</span>
            {question && <><span>·</span><span style={{ color: '#a78bfa' }}>{HEADING_LABELS[question.heading]}</span></>}
            <span>·</span>
            <span>{answer.gradingStatus === 'pending' ? 'Pending'
      ...
      {open && (
        <div style={{ padding: '0 12px 14px', borderTop: '1px solid var(--border-color)' }}>
          ...
          {question?.expectedAnswer && answer.gradingStatus === 'graded' && score < 100 && (
            <div style={{ marginTop: 10 }}>
              <p style={{ fontSize: 11, color: 'var(--text-muted)', margin: '0 0 4px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.4 }}>Model Answer</p>
              <p style={{ fontSize: 13, color: 'var(--text-primary)', margin: 0, lineHeight: 1.5 }}>{question.expectedAnswer}</p>
            </div>
          )}
        </div>
      )}
...
export default function EvolutionResults({ session, questions, onNewSession }: Props) {
  const qMap = useMemo(() => {
  ...
  const topicChartData = useMemo(() => {
    ...
    return Array.from(topicMap.entries()).map(([topic, { total, count }]) => ({
      ...
      color: TOPIC_COLORS[topic as keyof typeof TOPIC_COLORS] ?? '#a78bfa',
    }))
  }, [answers, qMap])
