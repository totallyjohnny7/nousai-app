/**
 * BiolSession — Active BIOL quiz session.
 * Handles MCQ radio buttons and free-response textareas,
...
export default function BiolSession({ session, questions, onFinish, onQuit }: Props) {
  ...
  const [answers, setAnswers]           = useState<Record<string, BiolAnswer>>(session.answers ?? {})
  ...
  const handleMcqSelect = useCallback(async (label: string) => {
    ...
    setGradingFeedback({ score, feedback: answer.feedback ?? '' })

    window.dispatchEvent(new CustomEvent('biol-grade-resolved', {
      detail: { questionId: currentId, score, feedback: answer.feedback },
    }))
...
Respond with ONLY a JSON object: {"score": <0-100>, "feedback": "<1-2 sentence feedback>"}`
      ...
      window.dispatchEvent(new CustomEvent('biol-grade-failed', {
      ...
      {(gradingFeedback || (alreadyAnswered && currentAnswer)) && (() => {
        ...
        return (
          <div style={{
            ...
            {currentAnswer?.gradingStatus === 'graded' && currentQ.explanation && currentAnswer.score < 100 && (
